...
require_once("system/class.define.php");
require_once("system/class.mobile.php");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    ...
    if ($col && $val !== null) {
        try {
            $db->table($selectedTable)->where($col, '=', $val)->delete();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    try {
        $db->table($selectedTable)->where($updateCol, '=', $updateVal)->update($data);
        header("Location: ?table=" . urlencode($selectedTable));
        exit;
    } catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['insert'])) {
    ...
    try {
        $db->table($selectedTable)->insert($data);
        header("Location: ?table=" . urlencode($selectedTable));
        exit;
    } catch (Exception $e) {
        ...
        <div class="content">
            ...
            <?php elseif ($selectedTable): ?>
                ...
                <?php if (isset($_GET['edit_column'], $_GET['edit_value'])): ?>
                    ...
                    $editingRow = $db->table($selectedTable)->where($editCol, '=', $editVal)->getRow('array');
                    ?>
                ...
                <?php
                $results = $db->table($selectedTable)->getAll('array');
